...
const Form = () => {
  ...
  const handleDelete = (id) => {
    // setData(data.filter((el) => el.id !=t= id));
  ...
  const handleSubmit = (e) => {
    e.preventDefault();
    axios.post("http://localhost:4000/form", form).then((res) => {
  ...
  useEffect(() => {
    axios
      .get(
        `http://localhost:4000/form?_sort=salary&_order=${sort}&_page=${page}&_limit=5`
      )
      .then((res) => {
        setData(res.data);
